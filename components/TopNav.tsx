...
export default function TopNav({
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 sm:gap-3.5 bg-white border border-black/[0.08] rounded-full px-2.5 sm:px-6 py-1.5 sm:py-2.5 shadow-sm max-w-[calc(100vw-24px)]">
      {/* Name */}
      <button
        onClick={() => onNavigate('home')}
        className="font-serif italic text-[#0A0A0A] text-[18px] sm:text-[28px] mr-0 sm:mr-1 shrink-0 group/name overflow-hidden"
      >
        <span className="grid">
      ...
      {iconItems.map((item) => (
        <div key={item.id} className="relative flex flex-col items-center gap-1 shrink-0 group">
          ...
          <button
            ...
            className={`w-9 h-9 sm:w-12 sm:h-12 rounded-[10px] sm:rounded-[12px] flex items-center justify-center text-base sm:text-2xl transition-colors duration-150 ${
